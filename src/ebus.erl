...
%% me-cek apakah Topik yang dicari termasuk yang di subscribe
isSubscribers(Topic) -> lists:any(Topic, subscribers(Topic)) -> ok | {error, term()}.

%% me-cek adakah messages yang tidak diterima oleh self()
isMissing(messages(Pid), messages(self())) -> lists:intersection(messages(Pid), messages(self())).
